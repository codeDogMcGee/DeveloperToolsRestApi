...
from .helpers import get_json_from_api, create_serialized_organizations_object, csv_response, DATA_URL
...
class OrganizationsCsvDownloadView(APIView):
    permission_classes = [permissions.AllowAny]
    ...
    def get(self, request):
        ...
        if data_response['status'] == 200 and 'releases' in data_from_source.keys():
            ...
            if len(organizations_json) > 0:
                ...
                return csv_response(organizations_json['organizations'])
...
class OrganizationsSortedCsvView(APIView):
    permission_classes = [permissions.AllowAny]
